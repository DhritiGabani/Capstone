...
CLEAN_SIGNALS_PATH = os.path.join(
    PROJECT_DIR, "results/clean_signals_with_features.csv")
...
def find_rep_boundaries(pitch_signal):
    ...
    """
    peaks, _ = find_peaks(pitch_signal, distance=1, prominence=1)
...
def remove_bad_reps_and_renumber(segmented_df):
    ...
    """
    bad_reps = [
        (2, "Heel Walk", "toe_high", [1]),
        (3, "Calf Raises", "10_fast", [10, 11]),
        (3, "Heel Walk", "toe_high", [11, 12]),
        (3, "Heel Walk", "toe_low", [1]),
        (4, "Calf Raises", "10_slow", [1, 2, 10]),
        (4, "Heel Walk", "toe_low", [1, 10, 11, 12, 13, 14, 15]),
        (6, "Heel Walk", "toe_low", [1, 13]),
        (7, "Ankle Rotation", "10_fast_CW", [1]),
        (7, "Calf Raises", "10_slow", [2, 4, 6, 8, 10, 12, 14, 16]),
        (7, "Heel Walk", "toe_low", [9]),
        (10, "Calf Raises", "10_slow", [1, 3, 5, 7, 9, 11, 13, 15, 17]),
        (10, "Heel Walk", "toe_high", [1, 5]),
        (11, "Heel Walk", "toe_low", [1]),
        (13, "Calf Raises", "10_fast", [1]),
        (13, "Heel Walk", "toe_high", [1]),
        (15, "Heel Walk", "toe_high", [1]),
        (19, "Calf Raises", "10_slow", [2, 4, 6, 8, 10, 12, 14, 16]),
        (22, "Ankle Rotation", "10_slow_CW", [6, 7, 8, 9]),
        (27, "Calf Raises", "10_fast", [1, 2]),
    ]
    ...
    renumbered_rows = []
    grouped = cleaned_df.groupby(
        ['subject_id', 'exercise', 'set', 'sensor_location', 'file_path'])
    ...
    cleaned_df = cleaned_df.sort_values(
        ['subject_id', 'exercise', 'set', 'sensor_location', 'rep']).reset_index(drop=True)
...
def segment_data(signals_df, meta_df):
    ...
    foot_files = meta_df[meta_df['sensor_location']
                         == 'foot']['file_path'].unique()
    ...
    for foot_file in foot_files:
        ...
        features_to_normalize = ['acc_x', 'acc_y', 'acc_z',
                                 'gyr_x', 'gyr_y', 'gyr_z', 'roll', 'pitch']
        ...
        shank_baseline_idx = (
            shank_data['time'] - fifth_rep_start_time).abs().idxmin()
